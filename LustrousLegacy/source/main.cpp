...
int main(int argc, char** argv) {
	...
	*********************************************************************/
	...
	sf::Music music;
	if (!music.openFromFile("resources/audio/LL_Town_Theme.wav"))
		return -1; // error
	...
	*********************************************************************/
	...
	Event tutorial = { { Direction::South,7} ,{ Direction::East,3 },{ Direction::North,1 } };
	bool fade_out = false;
	bool fade_in = false;
	...
	while (window.isOpen()) {
		...
		if (fade_out && !sysFader.isComplete())
		{
			...
			if (sysFader.isComplete()) {
				...
				sysFader.resetFader();
				event_start = true;
				tutorial = Event({ {Direction::South,1}, { Direction::East,3 }, { Direction::North,1 } });
			}
		}
